...
$(document).ready(function () {
  $(".step-form-div .step-next").click(function () {
    let a = $(this).parents(".step-form-div");
    a.removeClass("active_step");
    a.next().addClass("active_step");
  });
  $(".step-form-div .prev").click(function () {
    let a = $(this).parents(".step-form-div");
    a.removeClass("active_step");
...
function updateSelectedDate() {
  ...
  setTimeout(hideDatePicker, 100); // Delay hiding to ensure update completes
}
...
window.onload = function () {
  ...
  const formattedDate = `${monthName} ${today.getDate()}, ${today.getFullYear()}`;
  document.getElementById('selectedDateText').textContent = `${formattedDate}`;
};
// date and time js end

// material list slider start
$(document).ready(function () {
  $('.material-list-slider').slick({
    infinite: true,
    slidesToShow: 4,
    slidesToScroll: 1,
    speed: 3000,
...
// material list slider end

// material list slider start
$(document).ready(function () {
  $('.material-bundle-slider').slick({
    infinite: true,
    slidesToShow: 1,
    slidesToScroll: 1,
    speed: 3000,
...
// other material list slider start
...
// material list slider end
// other material list slider end
...
function openSideSwap(sectionId) {
  document.querySelector(".SideSwap").style.display = "block";
  body.style.overflow = 'hidden';
  const sidebarButtons = document.querySelectorAll(".swap-btn.sidebar");
  sidebarButtons.forEach(button => {
    const originalParams = button.getAttribute('onclick').split("','");
    const match = originalParams[0].match(/'([^']+)'/);
    const imageSource = match[1];
    const updatedParams = `'${imageSource}', '${sectionId}')`;
    button.setAttribute('onclick', `changeImg(${updatedParams}`);
  });
}
function closeSideSwap() {
  document.querySelector(".SideSwap").style.display = "none";
  body.style.overflow = 'auto';
}
function changeImg(img, sec) {
  const updatedImg = document.querySelector(`.changing-img.${sec}`);
  updatedImg.src = img;
  document.querySelector(".SideSwap").style.display = "none";
  body.style.overflow = 'auto';
}
...
$(document).ready(function () {
  var slider = $('.showdetial-img-slider').slick({
    infinite: true,
    slidesToShow: 1,
    slidesToScroll: 1,
    arrows: true,
  ...
  function updateSlideNumbers() {
    ...
    $('.showdetial-img-slider-numbers').html(`<span>${currentSlide + 1}</span> / ${slideCount}`);
  }
...
// more projects slider start
$(document).ready(function () {
  $('.more-proj-slider').slick({
    infinite: true,
    slidesToShow: 3,
    slidesToScroll: 1,
    arrows: true,
  ...
  $('.showdetial-img-slider').on('afterChange', function (event, slick, currentSlide) {
    // Get the total number of slides
    var slideCount = slick.slideCount;
    // Update the image numbering
    $('.showdetial-img-slider-numbers').html(`<span>${currentSlide + 1}</span> / ${slideCount}`);
...
// displaying uploaded images start
const uploadInput = document.getElementById('upload-img1');
  ...
  for (let i = 0; i < files.length; i++) {
    ...
    if (file) {
      ...
      reader.onload = function (e) {
        ...
        removeIcon.className = 'far fa-times';

        removeIcon.addEventListener('click', function () {
          uploadedImageContainer.querySelector('ul').removeChild(listItem);
        });

        listItem.appendChild(removeIcon);
        listItem.appendChild(image);
        uploadedImageContainer.querySelector('ul').appendChild(listItem);
      };
...
// displaying uploaded images end

// more images upload start
document.querySelectorAll('.upload-more input[type="file"]').forEach(input => {
